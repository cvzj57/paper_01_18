...
\hline
\raisebox{-.5\height}{\includegraphics[width=2cm]{alpha}}
& $\alpha$ & $sp^2$ 1-electron\\
\hline
\raisebox{-.5\height}{\includegraphics[width=2cm]{beta}}
& $\beta$ & $sp^2$ 1-electron \\
\hline
\raisebox{-.5\height}{\includegraphics[width=2cm]{gamma}}
& $\gamma$ & $sp^3$ 2-electron \\
\hline
\raisebox{-.5\height}{\includegraphics[width=3cm]{delta}}
...
In the $\gamma$ and $\delta$ set-ups, th distance from central pseudocrabon to non-atom-centered potentials is denoted by $c$. In the $\alpha$ and $\beta$ set-ups, the planar distance to each potential set is denoted by $c$, and the distance above and below the $xy$ plane by $d$. Figure~ref{fig:cddef} illustrates this.

\begin{figure}
\begin{center}
\includegraphics[width=5cm]{tm_sp2_potentials}
\caption{Definition of distances $c$ and $d$ for $\alpha$ and $]beta$ pseuodpotential set-ups}\label{fig:cddef}
\end{center}
\end{figure}
...
Figure~\ref{fig:beta_graphs} displays results for the HOMO energy, first ionisation energy, singlet-triplet gap energy and first excitation energy across the range of different test molecules using $\beta$ potentials. The majority of the results can be seen immediately to be well within half an electron-volt of the all-electron values. The pattern which emerges is that for systems where the potentials are bonded to carbon, the error is relativity small, but where the pseudocarbon bond is to another element such as oxygen (CH$_2$O), or nitrogen (CH$_2$NH), then the errors are larger, up to around 30\%. Given the potentials are optimised on a potential system bonded to a real carbon, this is probably to be expected. Bonds between carbon atoms have a very different character to bonds between carbon and oxygen or nitrogen atoms.
...
\hline
$p$ & \multicolumn{1}{c|}{def-SV(P)} & \multicolumn{2}{c|}{\makecell{ HOMO(*3);\\ HOMO-1;\\ HOMO-2\\}} & 0.90670 \\
\hline
\textbf{potential} & 
...
\hline
$s,p$ & \multicolumn{1}{c}{def-SV(P)} & \multicolumn{2}{|c|}{\makecell{HOMO($\times$2);\\ HOMO-1}} & 0.4179 \\
\hline
\textbf{Potential} & 
\multicolumn{1}{|c|}{\textbf{Coefficient}} & 
\multicolumn{1}{|c|}{\textbf{Exponent}} & \textbf{$d$ ($a.u.$)} & \textbf{$c$ ($a.u.$)} \\
...
\hline
\multicolumn{1}{|c|}{$s, p$} &
\multicolumn{1}{|c|}{def-SV(P)} & \multicolumn{2}{c|}{\makecell{HOMO;\\ HOMO-1;\\ HOMO-2;\\ HOMO-3;\\ HOMO-4;\\ HOMO-5}} & 0.40067 \\
\hline
...
\multicolumn{2}{c|}{\makecell{HOMO;\\ HOMO+1;\\ HOMO+2}} & 0.04037 \\
\hline
\textbf{Potential} & 
\multicolumn{1}{|c|}{\textbf{Coefficient}} & 
\multicolumn{1}{|c|}{\textbf{Exponent}} & 
...
\makecell{
\chemfig{[:40]-[@{left,.7}]C^\ast% 2
    =[:320]C^\ast% 3
     -[@{right,0.3}:40]
}
\makebraces[25pt,25pt]{\!\!\!n}{left}{right}} \\
all-trans-polyene \\
\includegraphics[width=6cm]{PAHs}\\
PAH \\
\end{tabular}
\end{center}
